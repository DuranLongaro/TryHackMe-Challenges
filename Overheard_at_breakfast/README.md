...
This is when i used `printf '%s' 'lambobytelotushotel@gmail.com' | sha256sum`. The `printf` outputs the email without adding a newline, then `|` pipes the email into `sha256sum`, which then generates the SHA-256 hash.

The resulting hash was used to query the Gravatar profile:

`curl https://gravatar.com/<SHA256_HASH>.json`

I used `curl` to make a request to the Gravatar JSON endpoint and the response contained profile information. Upon searching through the profile, I found the encoded version of the flag in the `about me` field. After copying the encoded version I used `echo '<BASE64_STRING>' | base64 -d` to decode using `base64`.
